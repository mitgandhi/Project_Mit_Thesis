...
def create_total_power_loss_plot(data, pressure_val=None, displacement_val=None, relative=True):
    """
    Create a grouped bar chart for total power losses
    X-axis: Operating Conditions (speeds), Grouped bars: Different Zeta values (colors)
    Y-axis: Power loss in % or W

    Parameters:
    - data: Dictionary with structure {zeta: {speed: {'mechanical_loss': val, 'volumetric_loss': val}}}
    - pressure_val: Pressure value for title
    - displacement_val: Displacement value for title
    - relative: If True, show as percentages; if False, show absolute values
    ...
    # Prepare data structure: [speed][zeta] = total_loss_value
    speed_zeta_data = {}
    for speed in speeds:
        speed_zeta_data[speed] = {}
        for zeta in zeta_values:
            if speed in data[zeta]:
                ...
                speed_zeta_data[speed][zeta] = total_loss
            else:
                speed_zeta_data[speed][zeta] = 0
    ...
    if relative:
        ...
        for speed in speeds:
            for zeta in zeta_values:
                grand_total += speed_zeta_data[speed][zeta]

        # Convert to percentages
        if grand_total > 0:
            for speed in speeds:
                for zeta in zeta_values:
                    speed_zeta_data[speed][zeta] = (speed_zeta_data[speed][zeta] / grand_total) * 100

    # Create the plot
    fig, ax = plt.subplots(figsize=(12, 8))

    # Define colors for different zeta values
    ...
    # Create grouped bars for each zeta value
    for i, zeta in enumerate(zeta_values):
        total_values = [speed_zeta_data[speed][zeta] for speed in speeds]

        # Position for this zeta group
        pos = x + (i - len(zeta_values) / 2 + 0.5) * width

        bars = ax.bar(pos, total_values, width,
                      label=f'Zeta {zeta}°',
                      color=colors[i % len(colors)], alpha=0.8)

        # # Add value labels below x-axis
        # if relative:
        #     for j, total in enumerate(total_values):
        #         if total > 2:  # Only show labels for values > 2%
        #             ax.text(pos[j], -5, f'{total:.1f}%',
        #                     ha='center', va='top',
        #                     fontweight='bold', fontsize=9)
        # else:
        #     for j, total in enumerate(total_values):
        #         if total > 0:
        #             ax.text(pos[j], -0.5, f'{total:.1f}W',
        #                     ha='center', va='top',
        #                     fontweight='bold', fontsize=9)
    ...
    if relative:
        ...
        # Set y-limit with space below for labels
        max_val = max([max(speed_zeta_data[speed].values()) for speed in speeds])
        ax.set_ylim(-10, max_val * 1.15 if max_val > 0 else 100)
    else:
        ax.set_ylabel('Power Loss (W)', fontweight='bold')
        max_val = max([max(speed_zeta_data[speed].values()) for speed in speeds])
        ax.set_ylim(-max_val * 0.1, max_val * 1.15 if max_val > 0 else 1)
    ...
    ax.set_title(
        f'Pressure: {pressure_val} bar, Displacement: {displacement_val} %')
    # else:
        # ax.set_title(f'{title_prefix}Total Power Loss Analysis')

    # Legend below x-axis with more gap
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3, frameon=True,
              ...
              fontsize=12, markerscale=1.5, columnspacing=2.0, handlelength=3.0,
              handletextpad=1.0, borderpad=1.5)
    ...
    # Save the figure (keeping the original save functionality)
    fig.savefig('power_loss_analysis.png', dpi=300, bbox_inches='tight')
...
def main(root_path, plot_type='relative', displacement_volume=56):
    ...
    try:
        ...
        if fig1 is not None and ax1 is not None:
            ...
            print("⚠️ Could not create power loss plot")
    except Exception as e:
        print(f"⚠️ Error creating power loss plot: {e}")
        import traceback
        traceback.print_exc()
    ...
    # except Exception as e:
    #     print(f"⚠️ Error creating all efficiency plots: {e}")
    ...
    #     except Exception as e:
    #         print(f"⚠️ Error creating {eff_type} efficiency plot: {e}")
    #
    ...
    # except Exception as e:
    #     print(f"⚠️ Error creating efficiency loss breakdown plot: {e}")
    #
