...
#                folder_w/2000/output/piston/piston.txt
#
#     Args:
...
#     Grouped bars: All optimization methods
#     """
#     plt.rcParams.update({
#         'font.size': 12,
#         'axes.labelsize': 14,
...
#     Args:
#         folder_paths: List of tuples [(folder_name, folder_path), ...]
#                      e.g., [("OPTIMAL-BAYESIAN", "/path/to/x"), ("OPTIMAL-NSGA", "/path/to/y"),
#                            ("BASE_MODEL", "/path/to/z"), ("EXPERIMENTAL", "/path/to/w")]
#     """
...
#         print("  folder_w/[speed_folders]/output/piston/piston.txt")
#         return
#
...
#         ("OPTIMAL-BAYESIAN", folder_x_path),
#         ("OPTIMAL-NSGA", folder_y_path),
#         ("BASE_MODEL", folder_z_path)
#     ]
#     main(folder_paths, plot_type)
#
#
# def main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path, plot_type='relative'):
#     """Convenience function for four folders with proper labels"""
#     folder_paths = [
#         ("Phase-I   (clearnance)", folder_x_path),
#         ("Phase-II  (Inclined_angle)", folder_y_path),
#         ("Phase-III (Length)", folder_z_path),
#         ("Standard-Design", folder_w_path)
#     ]
#     main(folder_paths, plot_type)
#
#
...
#     # ============================================================================
#
#     # Option 1: For FOUR folders with proper labels
#     folder_x_path = r"Z:\Studenten\Mit\Inline_Thesis-Simulation\HSP\RUN\Run12_Optimal_value\2_para"
#     folder_y_path = r"Z:\Studenten\Mit\Inline_Thesis-Simulation\HSP\RUN\Run12_Optimal_value\3_para_dK_19.332_dZ_19.352_zeta_8"
#     folder_z_path = r"Z:\Studenten\Mit\Inline_Thesis-Simulation\HSP\RUN\Run12_Optimal_value\5_para_dK_19.678_dZ_19.694_LKG_47_LF_32.5_zeta_4"
#     folder_w_path = r"Z:\Studenten\Mit\Inline_Thesis-Simulation\HSP\RUN\standard_design\standard_design"
#
#     # Run four-folder comparison with proper labels
#     main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path, plot_type='relative')
#
#     # ============================================================================
#
#     # Option 2: For THREE folders with proper labels (uncomment if you only have three folders)
#     # main_three_folders(folder_x_path, folder_y_path, folder_z_path, plot_type='relative')
#
#     # ============================================================================
#
#     # Option 3: For TWO folders with proper labels (uncomment if you only have two folders)
#     # main_two_folders(folder_x_path, folder_y_path, plot_type='relative')
#
#     # ============================================================================
#
#     # Option 4: For CUSTOM folder configuration with explicit labels
#     # folder_paths = [
...
#     #     ("CUSTOM_LABEL", r"Z:\path\to\your\fourth\folder"),
#     # ]
...
#     # main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path, plot_type='absolute')
#
#     print("\nAnalysis complete! The plots now support 4 folders with labels:")
#     print("  - X folder → Phase-I   (clearnance)")
#     print("  - Y folder → Phase-II  (Inclined_angle)")
#     print("  - Z folder → Phase-III (Length)")
#     print("  - W folder → Standard-Design")
                ...
                try:
                    ...
                    if df is not None:
                        ...
                        if pressure_cols:
                            ...
                            print(f"    Loaded existing data - Max: {max_pressure:.2e} Pa, Mean: {mean_pressure:.2e} Pa")
...
def read_contact_pressure_data_fallback(speed_folder_path):
    ...
    try:
        ...
        return None, None
# Part 3: Enhanced Data Processing Function

def process_multiple_folder_structure_with_contact_pressure(folder_paths):
    """
    Enhanced version that collects both power loss and contact pressure data for 4 folders
    """
    all_data = {}
        ...
        for speed_val in speed_folders:
            ...
            if os.path.exists(piston_file):
                ...
                if mech_loss is not None and vol_loss is not None:
                    ...
                    print(f"    Power Loss - Mech: {mech_loss:.2f}W, Vol: {vol_loss:.2f}W, Total: {mech_loss + vol_loss:.2f}W")
                    print(f"    Total Force: {total_force:.2f}N")
                    if max_contact_pressure is not None:
                        print(f"    Contact Pressure - Max: {max_contact_pressure:.2e}Pa, Mean: {mean_contact_pressure:.2e}Pa")
                    else:
    ...
    return all_data
# Part 4: Enhanced Plotting Functions
...
def create_contact_pressure_vs_total_loss_plot(all_data, target_speed=4500):
    ...
    for the specified speed across all optimization methods (LINEAR SCALE - supports 4 folders)
    """
    plt.rcParams.update({
        'font.size': 18,
        'axes.labelsize': 16,
    ...
    # Colors and markers for different folders - expanded for 4 folders
    colors = ['#2E86AB', '#A23B72', '#F18F01', '#42B883']  # Blue, Purple, Orange, Green
...
def create_detailed_breakdown_plot(all_data):
    """
    Create a detailed breakdown plot showing mechanical vs volumetric losses (supports 4 folders)
    """
    plt.rcParams.update({
        'font.size': 12,
        'axes.labelsize': 14,
    ...
    return fig, ax
# Part 5: Dual Column Plot Functions

def create_dual_column_plot(all_data, target_speed=4500):
    """
    Create a dual-column plot for 4 folders:
    Left: Total Loss Bar Chart for target speed
    Right: Contact Pressure vs Total Loss Scatter Plot for target speed
    """
    plt.rcParams.update({
        'font.size': 12,
        'axes.labelsize': 14,
    ...
    if not plot_data:
        ...
        ax1.set_title(f'Total Loss Comparison\n(Speed: {target_speed} RPM, Pressure: 320bar, Displacement: 100%)', fontweight='bold', fontsize=14)
        ax2.set_title(f'Max Contact Pressure vs Total Loss\n(Speed: {target_speed} RPM, Pressure: 320bar, Displacement: 100%)', fontweight='bold',
                      fontsize=14)
    ...
    # RIGHT SUBPLOT: Scatter plot of contact pressure vs total loss
    markers = ['o', '^', 's', 'D']  # Circle, Triangle, Square, Diamond
    folder_markers = {folder: markers[i % len(markers)] for i, folder in enumerate(folder_names)}
    ...
    for i, data in enumerate(plot_data):
        ...
        ax2.scatter(total_loss, max_contact_pressure,
                    color=folder_colors[folder_name],
                    marker=folder_markers[folder_name],
                    s=200, alpha=0.8,
                    label=folder_name,
        ...
        # # Add text annotation
        # ax2.annotate(folder_name,
        #              (total_loss, max_contact_pressure),
        #              xytext=(10, 10), textcoords='offset points',
        #              fontsize=11, fontweight='bold')

    ax2.set_xlabel('Total Loss [W]', fontweight='bold')
    ...
    # Save the plot
    filename = f'dual_column_analysis_{target_speed}rpm.png'
    fig.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"Dual-column plot saved as: {filename}")
...
def create_contact_pressure_vs_volumetric_loss_plot(all_data, target_speed=4500):
    ...
    for the specified speed across all optimization methods (supports 4 folders)
    """
    plt.rcParams.update({
        'font.size': 14,
        'axes.labelsize': 16,
    ...
    plt.tight_layout()
    fig.savefig(f'contact_pressure_vs_volumetric_loss_{target_speed}rpm.png', dpi=300,
                bbox_inches='tight')

    return fig, ax, plot_data
def generate_dual_column_plot_only(folder_paths, target_speed=4500):
    """
    Generate only the dual-column plot for 4 folders
    """
    print(f"Processing data for dual-column plot at {target_speed} RPM...")
    ...
    print(f"Found data for folders: {list(all_data.keys())}")

    # Create the dual-column plot
    fig, axes, plot_data = create_dual_column_plot(all_data, target_speed=target_speed)

    if fig is not None:
        plt.show()
        print(f"Dual-column plot created for {target_speed} RPM")

        # Print summary
        if plot_data:
            print(f"\nData Summary for {target_speed} RPM:")
            for data in plot_data:
                print(f"  {data['folder_name']}: Total Loss={data['total_loss']:.2f}W, "
                      f"Max Contact Pressure={data['max_contact_pressure']:.2e}Pa")
        else:
            print(f"No complete data available for {target_speed} RPM")

        return fig, axes, plot_data
    else:
        print("Failed to create dual-column plot")
        return None


def test_dual_column_plot_only(folder_paths, target_speed=4500):
    """
    Test the dual-column plot with detailed debugging for 4 folders
    """
    print("=" * 80)
    print(f"TESTING DUAL-COLUMN PLOT CREATION FOR {target_speed} RPM")
    print("=" * 80)

    print("Step 1: Processing folder data...")
    all_data = process_multiple_folder_structure_with_contact_pressure(folder_paths)

    if not all_data or not any(all_data.values()):
        print("ERROR: No data found. Check folder structure and file paths.")
        return None

    print(f"SUCCESS: Found data for folders: {list(all_data.keys())}")

    print(f"\nStep 2: Checking data availability for speed {target_speed} RPM...")
    available_data = {}
    for folder_name, folder_data in all_data.items():
        if target_speed in folder_data:
            speed_data = folder_data[target_speed]
            has_loss = 'total_loss' in speed_data and speed_data['total_loss'] is not None
            has_cp = 'max_contact_pressure' in speed_data and speed_data['max_contact_pressure'] is not None

            available_data[folder_name] = {
                'has_loss': has_loss,
                'has_contact_pressure': has_cp,
                'total_loss': speed_data.get('total_loss'),
                'max_contact_pressure': speed_data.get('max_contact_pressure')
            }

            print(f"  {folder_name}: Loss={has_loss} ({speed_data.get('total_loss'):.2f}W if available), "
                  f"Contact Pressure={has_cp} ({speed_data.get('max_contact_pressure'):.2e}Pa if available)")
        else:
            print(f"  {folder_name}: No data for speed {target_speed} RPM")

    if not available_data:
        print(f"ERROR: No folders have data for speed {target_speed} RPM")
        return None

    complete_data_count = sum(1 for data in available_data.values()
                              if data['has_loss'] and data['has_contact_pressure'])
    print(f"\nStep 3: Found {complete_data_count} folders with complete data")

    if complete_data_count == 0:
        print("WARNING: No folders have both loss and contact pressure data")

    print(f"\nStep 4: Creating dual-column plot...")
    try:
        fig, axes, plot_data = create_dual_column_plot(all_data, target_speed=target_speed)

        if fig is not None:
            print("SUCCESS: Dual-column plot created")
            print("Step 5: Displaying plot...")
            plt.show()

            print(f"\nStep 6: Final summary for {target_speed} RPM:")
            if plot_data:
                for data in plot_data:
                    print(f"  ✓ {data['folder_name']}: Loss={data['total_loss']:.2f}W, "
                          f"Max CP={data['max_contact_pressure']:.2e}Pa")
            else:
                print("  No complete plot data available")

            return fig, axes, plot_data
        else:
            print("ERROR: Failed to create dual-column plot")
            return None

    except Exception as e:
        print(f"ERROR: Exception during plot creation: {e}")
        import traceback
        traceback.print_exc()
        return None
# Part 6: Enhanced Summary and Main Functions

def create_summary_comparison_table(all_data):
    """
    Create a comprehensive summary table including contact pressure data for 4 folders
    """
    print("\n" + "=" * 180)  # Increased width for 4 folders
...
def create_dual_column_plot_volumetric(all_data, target_speed=4500):
    """
    Create a dual-column plot for 4 folders:
    Left: Volumetric Loss Bar Chart for target speed
    ...
    if not plot_data:
        ...
        ax1.set_title(f'Total Loss Comparison\n(Speed: {target_speed} RPM, Pressure: 320bar, Displacement: 100%)', fontweight='bold', fontsize=14)
        ax2.set_title(f'Max Contact Pressure vs Volumetric Loss\n(Speed: {target_speed} RPM, Pressure: 320bar, Displacement: 100%)', fontweight='bold',
                      fontsize=14)

        plt.tight_layout()
        return fig, (ax1, ax2), []

    # LEFT SUBPLOT: Bar chart of volumetric losses
    bars = ax1.bar(folder_labels, total_losses, color=bar_colors, alpha=0.8, edgecolor='black', linewidth=1)
...
def main(folder_paths, plot_type='relative', target_speed=4500):
    """
    Main function to process multiple folders and create comparative visualization
    with integrated contact pressure analysis using T1 methods (supports 4 folders)
    """
    ...
    # Create and show the comparative power loss plot
    print("\nCreating Comparative Power Loss Plot...")
    try:
        fig1, ax1 = create_comparative_power_loss_plot(all_data, relative=relative)
        if fig1 is not None and ax1 is not None:
            plt.figure(fig1.number)
    ...
    # Create contact pressure vs total loss plot
    print(f"\nCreating Contact Pressure vs Total Loss Plot for {target_speed} RPM...")
    try:
    ...
    # # Create dual-column plot
    # print(f"\nCreating Dual-Column Plot for {target_speed} RPM...")
    # try:
    #     fig4, axes4, dual_data = create_dual_column_plot(all_data, target_speed=target_speed)
    #     if fig4 is not None and axes4 is not None:
    #         plt.figure(fig4.number)
    #         plt.show()
    #         print("Dual-column plot displayed successfully")
    #
    #         # Print summary
    #         if dual_data:
    #             print(f"\nDual-Column Analysis Summary for {target_speed} RPM:")
    #             for data in dual_data:
    #                 print(f"  {data['folder_name']}: Total Loss={data['total_loss']:.2f}W, "
    #                       f"Max Contact Pressure={data['max_contact_pressure']:.2e}Pa")
    #         else:
    #             print(f"No complete data available for dual-column plot at {target_speed} RPM")
    #     else:
    #         print("⚠️ Could not create dual-column plot")
    # except Exception as e:
    #     print(f"⚠️ Error creating dual-column plot: {e}")
    #     import traceback
    #     traceback.print_exc()

    # Create dual-column plot with volumetric loss
    print(f"\nCreating Dual-Column Plot (Volumetric Focus) for {target_speed} RPM...")
    try:
        ...
        import traceback
        traceback.print_exc()
    # Create contact pressure vs volumetric loss plot
    print(f"\nCreating Contact Pressure vs Volumetric Loss Plot for {target_speed} RPM...")
    try:
    ...
    # print(f"\nCreating Dual-Column Plot (Volumetric Focus) for {target_speed} RPM...")
    # try:
    #     fig6, axes6, dual_data = create_dual_column_plot_volumetric(all_data, target_speed=target_speed)
    #     if fig4 is not None and axes4 is not None:
    #         plt.figure(fig4.number)
    #         plt.show()
    #         print("Dual-column volumetric plot displayed successfully")
...
def main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path, plot_type='relative', target_speed=4500):
    """Convenience function for four folders with proper labels"""
    folder_paths = [
        ("Phase-I   (clearnance)", folder_x_path),
        ("Phase-II  (Inclined_angle)", folder_y_path),
...
if __name__ == "__main__":
    ...
    # Run four-folder comparison with contact pressure analysis
    print("=" * 80)
    print("INTEGRATED POWER LOSS AND CONTACT PRESSURE ANALYSIS (4 FOLDERS)")
    print("=" * 80)
    ...
    main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path,
    ...
    # Option 2: For THREE folders with contact pressure analysis
    # main_three_folders(folder_x_path, folder_y_path, folder_z_path, plot_type='relative', target_speed=4500)

    # Option 3: For TWO folders with contact pressure analysis
    # main_two_folders(folder_x_path, folder_y_path, plot_type='relative', target_speed=4500)

    # Option 4: For CUSTOM folder configuration with different target speed
    # folder_paths = [
    #     ("Custom-Method-1", r"Z:\path\to\method1"),
    #     ("Custom-Method-2", r"Z:\path\to\method2"),
    #     ("Custom-Method-3", r"Z:\path\to\method3"),
    #     ("Baseline", r"Z:\path\to\baseline"),
    # ]
    # main(folder_paths, plot_type='relative', target_speed=2000)

    # Option 5: For absolute value plots instead of relative percentages
    # main_four_folders(folder_x_path, folder_y_path, folder_z_path, folder_w_path, plot_type='absolute', target_speed=4500)

    # Option 6: Generate ONLY the dual-column comparison plot for 4 folders
    # folder_paths = [
    #     ("Phase-I   (clearnance)", folder_x_path),
    #     ("Phase-II  (Inclined_angle)", folder_y_path),
    #     ("Phase-III (Length)", folder_z_path),
    #     ("Standard-Design", folder_w_path)
    # ]
    # result = generate_dual_column_plot_only(folder_paths, target_speed=4500)

    # Option 7: TEST the dual-column plot with detailed debugging for 4 folders
    # folder_paths = [
    #     ("Phase-I   (clearnance)", folder_x_path),
    #     ("Phase-II  (Inclined_angle)", folder_y_path),
    #     ("Phase-III (Length)", folder_z_path),
    #     ("Standard-Design", folder_w_path)
    # ]
    # result = test_dual_column_plot_only(folder_paths, target_speed=4500)

    print("\n" + "=" * 80)
    print("ANALYSIS COMPLETE (4 FOLDERS SUPPORTED)")
    print("=" * 80)
    print("Generated plots:")
    print("  1. Comparative power loss analysis (relative/absolute) - 4 folders")
    print("  2. Detailed breakdown (mechanical vs volumetric) - 4 folders")
    print("  3. Contact pressure vs total loss scatter plot - 4 folders")
    print("  4. Dual-column comparison (Total Loss Bar | Contact Pressure vs Total Loss Scatter) - 4 folders")
    print("All plots saved as PNG files in the current directory")
    print("\nFEATURES:")
    print("  ✓ Supports 4 optimization methods/folders")
    print("  ✓ Integrated T1 contact pressure analysis")
    print("  ✓ Fallback contact pressure methods")
    print("  ✓ Force calculations from power losses")
    print("  ✓ Comprehensive summary tables")
    print("  ✓ Multiple visualization types")
    print("\nTROUBLESHOOTING:")
    print("If contact pressure analysis fails:")
    print("1. Ensure T1 module is available in the same directory")
    print("2. Check required files exist: geometry.txt, operatingconditions.txt, Piston_Contact_Pressure.txt")
    print("3. Use Option 7 above for detailed debugging")
    print("4. Fallback methods will attempt to extract data from piston.txt")
    print("=" * 80)