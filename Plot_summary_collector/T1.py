...
def create_smooth_comparison_plots(results, base_folder):
    """
    Create smooth comparison plots between optimized and industrial design simulations.
    Uses interpolation and smoothing techniques for better visual quality.
    ...
    """
    print(f"\nCreating smooth comparison plots for {base_folder}")
    ...
    # Create smooth comparison plots for each matching simulation
    for sim_name in matching_sims:
        ...
        sigma = 1.0  # Adjust this value to control smoothness (higher = smoother)
        opt_smooth = gaussian_filter(opt_cumulative, sigma=sigma)
        ind_smooth = gaussian_filter(ind_cumulative, sigma=sigma)

        # Method 2: High-resolution interpolation
        # Create higher resolution grids for smoother visualization
        original_m, original_n = opt_cumulative.shape

        # Increase resolution by factor of 3
        smooth_factor = 3
        new_m = original_m * smooth_factor
        new_n = original_n * smooth_factor

        # Original coordinate grids
        original_y = np.linspace(0, original_n / 10000, original_n)
        original_x = np.linspace(0, 360, original_m)

        # High-resolution coordinate grids
        smooth_y = np.linspace(0, original_n / 10000, new_n)
        smooth_x = np.linspace(0, 360, new_m)

        # Create meshgrids for interpolation
        orig_X, orig_Y = np.meshgrid(original_y, original_x)
        smooth_X, smooth_Y = np.meshgrid(smooth_y, smooth_x)

        # Interpolate to higher resolution using cubic interpolation
        from scipy.interpolate import RectBivariateSpline

        # Create spline interpolators
        opt_spline = RectBivariateSpline(original_x, original_y, opt_cumulative, kx=3, ky=3, s=0)
        ind_spline = RectBivariateSpline(original_x, original_y, ind_cumulative, kx=3, ky=3, s=0)

        # Evaluate on high-resolution grid
        opt_smooth_interp = opt_spline(smooth_x, smooth_y)
        ind_smooth_interp = ind_spline(smooth_x, smooth_y)

        # Apply additional Gaussian smoothing to interpolated data
        opt_final = gaussian_filter(opt_smooth_interp, sigma=0.5)
        ind_final = gaussian_filter(ind_smooth_interp, sigma=0.5)

        # === SMOOTH CUMULATIVE PRESSURE COMPARISON ===
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 8))

        # Use higher DPI and smooth interpolation
        interpolation_method = 'bilinear'  # Options: 'bilinear', 'bicubic', 'gaussian'

        # Optimized plot (smooth version)
        im1 = ax1.imshow(opt_final,
                         extent=[0, original_n / 10000, 0, 360],
                         aspect='auto',
                         origin='lower',
                         cmap='jet',
                         interpolation=interpolation_method,
                         vmin=opt_result['minplot'],
                         vmax=opt_result['maxplot'])

        ax1.set_title(f'Optimized - Cumulative Pressure (Smooth)\n{sim_name}', fontsize=14, fontweight='bold')
        ax1.set_xlabel('Gap Length [m]', fontsize=12)
        ax1.set_ylabel('Gap Circumference [°]', fontsize=12)
        ax1.set_yticks([0, 60, 120, 180, 240, 300, 360])
        ax1.set_ylim([0, 360])
        cbar1 = plt.colorbar(im1, ax=ax1, label='Cumulative Pressure [Pa]')
        cbar1.ax.tick_params(labelsize=10)

        # Industrial plot (smooth version)
        im2 = ax2.imshow(ind_final,
                         extent=[0, original_n / 10000, 0, 360],
                         aspect='auto',
                         origin='lower',
                         cmap='jet',
                         interpolation=interpolation_method,
                         vmin=ind_result['minplot'],
                         vmax=ind_result['maxplot'])

        ax2.set_title(f'Industrial Design - Cumulative Pressure (Smooth)\n{sim_name}', fontsize=14, fontweight='bold')
        ax2.set_xlabel('Gap Length [m]', fontsize=12)
        ax2.set_ylabel('Gap Circumference [°]', fontsize=12)
        ax2.set_yticks([0, 60, 120, 180, 240, 300, 360])
        ax2.set_ylim([0, 360])
        cbar2 = plt.colorbar(im2, ax=ax2, label='Cumulative Pressure [Pa]')
        cbar2.ax.tick_params(labelsize=10)

        plt.tight_layout()
        plt.savefig(os.path.join(sim_comparison_dir, f'{sim_name}_Smooth_Cumulative_Pressure_Comparison.png'),
                    dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()

        # === ALTERNATIVE: CONTOUR PLOT VERSION ===
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 8))

        # Create contour plots for even smoother appearance
        levels = 20  # Number of contour levels

        # Optimized contour plot
        cs1 = ax1.contourf(smooth_Y, smooth_X, opt_final, levels=levels, cmap='jet',
                           vmin=opt_result['minplot'], vmax=opt_result['maxplot'])
        ax1.contour(smooth_Y, smooth_X, opt_final, levels=levels, colors='black', linewidths=0.2, alpha=0.3)

        ax1.set_title(f'Optimized - Cumulative Pressure (Contour)\n{sim_name}', fontsize=14, fontweight='bold')
        ax1.set_xlabel('Gap Length [m]', fontsize=12)
        ax1.set_ylabel('Gap Circumference [°]', fontsize=12)
        ax1.set_yticks([0, 60, 120, 180, 240, 300, 360])
        ax1.set_ylim([0, 360])
        ax1.set_xlim([0, original_n / 10000])
        cbar1 = plt.colorbar(cs1, ax=ax1, label='Cumulative Pressure [Pa]')
        cbar1.ax.tick_params(labelsize=10)

        # Industrial contour plot
        cs2 = ax2.contourf(smooth_Y, smooth_X, ind_final, levels=levels, cmap='jet',
                           vmin=ind_result['minplot'], vmax=ind_result['maxplot'])
        ax2.contour(smooth_Y, smooth_X, ind_final, levels=levels, colors='black', linewidths=0.2, alpha=0.3)

        ax2.set_title(f'Industrial Design - Cumulative Pressure (Contour)\n{sim_name}', fontsize=14, fontweight='bold')
        ax2.set_xlabel('Gap Length [m]', fontsize=12)
        ax2.set_ylabel('Gap Circumference [°]', fontsize=12)
        ax2.set_yticks([0, 60, 120, 180, 240, 300, 360])
        ax2.set_ylim([0, 360])
        ax2.set_xlim([0, original_n / 10000])
        cbar2 = plt.colorbar(cs2, ax=ax2, label='Cumulative Pressure [Pa]')
        cbar2.ax.tick_params(labelsize=10)

        plt.tight_layout()
        plt.savefig(os.path.join(sim_comparison_dir, f'{sim_name}_Smooth_Contour_Cumulative_Pressure_Comparison.png'),
                    dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()

        # === SMOOTH DIFFERENCE PLOT ===
        pressure_diff_smooth = gaussian_filter(opt_final - ind_final, sigma=0.5)

        plt.figure(figsize=(12, 8))

        # Use contourf for smooth difference plot
        diff_levels = 21  # Odd number for symmetric colormap
        cs = plt.contourf(smooth_Y, smooth_X, pressure_diff_smooth, levels=diff_levels, cmap='RdBu_r')
        plt.contour(smooth_Y, smooth_X, pressure_diff_smooth, levels=diff_levels, colors='black', linewidths=0.1,
                    alpha=0.4)

        cbar = plt.colorbar(cs, label='Pressure Difference [Pa]\n(Optimized - Industrial)')
        cbar.ax.tick_params(labelsize=10)

        plt.title(f'Pressure Difference: Optimized - Industrial (Smooth)\n{sim_name}', fontsize=14, fontweight='bold')
        plt.xlabel('Gap Length [m]', fontsize=12)
        plt.ylabel('Gap Circumference [°]', fontsize=12)
        plt.yticks([0, 60, 120, 180, 240, 300, 360])
        plt.ylim([0, 360])
        plt.xlim([0, original_n / 10000])

        plt.tight_layout()
        plt.savefig(os.path.join(sim_comparison_dir, f'{sim_name}_Smooth_Pressure_Difference.png'),
                    dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()

        print(f"  ? Created smooth comparison plots for {sim_name}")

    print(f"? All smooth comparison plots saved to {comparison_dir}")


# Modified version of the original function to include smooth plotting option
def create_comparison_plots_enhanced(results, base_folder, smooth=True):
    """
    Enhanced version that creates both original and smooth comparison plots.
    ...
    base_folder : str
        Base folder path where comparison plots will be saved
    smooth : bool
        Whether to create smooth versions of the plots
    """

    # Create original comparison plots
    create_comparison_plots(results, base_folder)

    # Create smooth comparison plots if requested
    if smooth:
        create_smooth_comparison_plots(results, base_folder)
...
def run_contact_pressure_analysis(base_folders, plots=360, degstep=1, ignore=0, minplot=0, maxplot=0):
    ...
    for base_folder, folder_results in results_by_base.items():
        ...
        if has_optimized and has_industrial:
            ...
            print("=" * 50)
            create_comparison_plots_enhanced(folder_results, base_folder)
